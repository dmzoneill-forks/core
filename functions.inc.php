...
function core_do_get_config($engine) {
	...
	*/
	...
	$exten = 's';
	$ext->add($context, $exten, '', new ext_authenticate('${ARG1}'));
	...
	*/
	...
	if (!empty($trunk_type_needed[$context])) {
		...
		$ext->add($context, $exten, '', new ext_execif('$["${QCALLBACK}" = "1"]', 'Set', 'DIAL_OPTIONS=${STRREPLACE(DIAL_OPTIONS,T)}'));
		$ext->add($context, $exten, '', new ext_gosubif('$[$["${ARG3}" != ""] & $["${DB(AMPUSER/${AMPUSER}/pinless)}" != "NOPASSWD"]]','sub-pincheck,s,1','','${ARG3}'));
		 ...
		 */
		$ext->add($context, $exten, '', new ext_gosub(1,'${DIAL_NUMBER}','trunk-dial-with-exten'));
		$ext->add('trunk-dial-with-exten', '_X','', new ext_dial('${OUT_${DIAL_TRUNK}}/${OUTNUM}${OUT_${DIAL_TRUNK}_SUFFIX}', '${TRUNK_RING_TIMER},${DIAL_TRUNK_OPTIONS}b(func-apply-sipheaders^s^1,(${DIAL_TRUNK}))'.$obroute_email));
